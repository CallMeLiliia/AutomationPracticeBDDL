...
public final class DriverHelperNew {
	private String mainWindowhandle;
	private WebDriver driver;
	private EventFiringWebDriver eDriver;

	public DriverHelperNew(WebDriver driver) {
		this.driver = driver;
	}

	public DriverHelperNew(EventFiringWebDriver driver) {
		this.eDriver = driver;
	}
...
//.get(index) return an object of WebElement
	...
	public void sendKeys(By by, String text, int timeOutInSeconds) {
		...
		ReporterNew.step.info("Typed: " + text);
	}
	...
	public void switchToNewWindow() {
		...
		for (String h : handles) {
			...
			if (h.equals(mainWindowhandle)) {
				...
				System.out.println("driver will notswitch");
			} else {
...
// js.executeScript("arguments[0].scrollIntoView(true)",webElement);
	...
	public void highlightElement(By by) {
		...
		WebElement webElement = getElement(by, 15);
		executeJS("arguments[0].style.border='3px dottedred'", webElement);
	...
	public void validateEquals(By by, String expectedText) {
		if (!getText(by, 15).equals(expectedText)) {
			...
			highlightElement(by);
			takeScreenshot();
			Assert.assertEquals(getText(by, 15),
